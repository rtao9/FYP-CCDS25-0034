...
from mmagent.utils.chat_api import parallel_get_embedding, generate_messages, get_response_with_retry
from mmagent.utils.general import validate_and_fix_json
from mmagent.utils.prompts import prompt_generate_full_memory
...
def generate_video_context(
    ...
    for char_id, faces in faces_list.items():
        ...
        # DEBUG: Check if frame_id exists
        if "frame_id" not in face:
            print(f"KeyError DEBUG: char_id {char_id} is missing 'frame_id'. Keys present: {list(face.keys())}")
            # Try to find a face in the list that DOES have a frame_id
            valid_face = next((f for f in faces if "frame_id" in f), None)
            if not valid_face:
                continue
            face = valid_face
        ...
        face_only.append((f"<face_{char_id}>:", face["metadata"]["face_thumb"]))
...
def generate_full_memories(base64_video, base64_frames, faces_list, voices_list):
    video_context = generate_video_context(base64_video, base64_frames, faces_list, voices_list)
    input = video_context + [
        {
            "type": "text",
...
def process_memories(video_graph, memory_contents, clip_id, type='episodic'):
    def get_memory_embeddings(memory_contents):
        ...
        embeddings = parallel_get_embedding(memory_contents)[0]
        return embeddings
